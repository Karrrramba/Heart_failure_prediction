...
set.seed(16633)
class(hf_train$death)

evaluate_model <- function(model, data) {

  cv_results <- model %>%
    vfold_cv(metric = "roc_auc", folds = 10)
...
test_vars <- function(df, target,  variable) {
  ...
  #   fit(data = df)
  null_model = glm(formula = as.factor(target) ~ 1, data = df, family = binomial(link = "logit"))
...
variable_results <- lapply(names(hf_train[, sapply(hf_train, is.factor)])[-which(names(hf_train) == "death")], 
