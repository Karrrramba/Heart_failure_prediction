...
hf <- hf %>% 
  ...
  mutate(death = fct_recode(death, deceased = "1", alive = "0"))
```
...
preds_transformed <- recipe(death ~ ., data = hf_train) %>% 
  step_YeoJohnson(time, platelets, serum_creatinine) %>%
  step_log(creatinine_phosphokinase, ejection_fraction) %>%
  step_sqrt(age, time) %>%
...
evaluate_model <- function(model, data) {
  ...
  cv_results <- model %>%
    rsample::cross_val(metric = "roc_auc", folds = 10)
...
# Function to test variable importance
test_vars <- function(df, target,  variable) {
  
  model <- logistic_reg() %>% 
    set_engine("glm")
  
  # null_formula <- as.formula(paste0(target, " ~ 1"))
  # 
  # null_model <- workflow(null_formula, model) %>%
  #   fit(data = df)
  null_model = glm(formula = target ~ 1, data = df, family = binomial(link = "logit"))
  
  alt_formula <- as.formula(paste0(target, " ~ ", variable))
  
  alt_model <- workflow(alt_formula, model) %>%
    fit(data = df)
  
  
  # Evaluate both models
  null_auc <- evaluate_model(null_model, df)
  model_auc <- evaluate_model(model, df)
  
  # Calculate improvement in ROC AUC
  improvement <- model_auc - null_auc
  
  # Calculate t-statistic for improvement
  t_stat <- (improvement * sqrt(10)) / sqrt(2 * (1 - improvement)^2)
  
  return(c(variable = variable, delta_auc = improvement, t_statistic = t_stat))
}

# Loop through each variable
variable_results <- lapply(names(hf_train)[-which(names(hf_train) == "death")], 
                           FUN = test_vars, df = hf_train, target = "death")

variable_results <- do.call(rbind, variable_results)
